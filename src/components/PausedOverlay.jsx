...
const PausedOverlay = ({ 
  ...
  return (
    <div className="absolute inset-0 z-[60] flex flex-col justify-center px-8 md:px-16 lg:px-24 animate-in fade-in duration-700 pointer-events-none overflow-hidden">
      {/* Soft cinematic scrim — keeps controls/video readable underneath */}
      <div className="absolute inset-0 bg-gradient-to-r from-black/90 via-black/50 to-transparent" />
      <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />

      <div className="relative z-10 max-w-2xl">
        <p className="text-xs md:text-sm font-medium tracking-[0.25em] uppercase text-white/50 mb-4">You're watching</p>

        {logoUrl ? (
          <img
            src={logoUrl}
            alt={title}
            className="max-w-[70vw] md:max-w-md max-h-24 md:max-h-32 object-contain object-left drop-shadow-2xl mb-5"
            onError={(e) => { e.currentTarget.style.display = 'none'; }}
          />
        ) : (
          <h1 className="text-white text-4xl md:text-6xl font-bold leading-tight tracking-tight drop-shadow-2xl mb-5">{title}</h1>
        )}

        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm md:text-base">
          {cleanQuality && (
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-white/10 border border-white/15 text-white text-xs font-semibold uppercase tracking-wide">
              <ShieldCheck size={14} /> {cleanQuality}
            </span>
          )}
          {imdbRating && imdbRating !== "0.0" && (
            <span className="inline-flex items-center gap-1.5 text-white font-semibold">
              <Star size={16} fill="#EAB308" className="text-yellow-500" /> {imdbRating}
            </span>
          )}
        ...
        <div className="flex items-center gap-2 mt-8 text-white/40 text-sm">
          <PlayCircle size={20} /> <span>Tap or press space to resume</span>
        </div>
      </div>
    </div>
  );
