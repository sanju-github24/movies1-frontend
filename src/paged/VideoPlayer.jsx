...
        if (!videoEl) return;
        
        // This button click now primarily acts as a toggle for the slider/mute itself
        if (showVolumeSlider) {
...
        } else {
            // If the slider is hidden, open it, unless already muted, then unmute
            if (isMuted) {
                videoEl.muted = false;
                setIsMuted(false);
                setVolume(volume || 1); 
...
             const videoEl = videoRef.current;
             if (!videoEl) return;
             // Toggle mute if clicking background area
...
                            return heightB - heightA; // Sort descending by resolution height
                        })
                        .map((level) => (
                            <li
                                key={level.id}
...
            {/* AUDIO/SUBTITLE SELECTION PANEL (Slides from right) */}
            <div 
...
                </h3>
                <ul className="mb-6 space-y-1 text-white border-b border-gray-700 pb-4">
                    {/* Default/Auto Option */}
                    <li 
...
                {/* Subtitles/Captions Section (Placeholder for full implementation) */}
                <h3 className="text-white text-md font-semibold mb-2 flex items-center">
                    <Equal className="w-4 h-4 mr-2" /> Subtitles (CC)
                </h3>
                <ul className="space-y-1 text-white">
                    {/* Subtitle Options (Example Structure) */}
                    <li className={`flex items-center justify-between p-3 rounded-lg cursor-pointer transition hover:bg-gray-700 bg-gray-700`}>
                        Disabled (Currently Selected)
                        <Check className="w-5 h-5 ml-auto" />
                    </li>
                    <li className={`flex items-center justify-between p-3 rounded-lg cursor-pointer transition hover:bg-gray-700`}>
                        English
                    </li>
                </ul>
            </div>
