  ...
  if (isEditing) {
    return (
      <div className="p-6 bg-slate-900 rounded-[2.5rem] border-2 border-blue-500/50 space-y-6 shadow-2xl animate-in zoom-in-95 overflow-hidden">
        ...
        <div className="grid grid-cols-2 gap-4">
            <div className="bg-slate-800/50 p-4 rounded-2xl border border-white/5 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div><p className="text-[10px] font-black text-white uppercase">Hero</p></div>
                </div>
                <button onClick={() => setEditData({...editData, show_on_hero: !editData.show_on_hero})} className={`w-10 h-5 rounded-full transition-all relative ${editData.show_on_hero ? 'bg-green-600' : 'bg-slate-700'}`}><div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${editData.show_on_hero ? 'right-1' : 'left-1'}`} /></button>
            </div>
            <div className="bg-slate-800/50 p-4 rounded-2xl border border-white/5 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <TrendingUp size={18} className={editData.is_trending ? "text-blue-400" : "text-slate-500"}/>
                    <div><p className="text-[10px] font-black text-white uppercase">Trending</p></div>
                </div>
                <button onClick={() => setEditData({...editData, is_trending: !editData.is_trending})} className={`w-10 h-5 rounded-full transition-all relative ${editData.is_trending ? 'bg-blue-600' : 'bg-slate-700'}`}><div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${editData.is_trending ? 'right-1' : 'left-1'}`} /></button>
            </div>
        </div>
                ...
                {editEpisodes.map((ep, idx) => (
                    <div key={idx} className="p-4 bg-slate-950 rounded-2xl border border-white/5 space-y-3 shadow-inner">
                        <div className="flex gap-2 items-center">
                            <input className="w-12 bg-slate-900 border border-slate-800 p-2 rounded-lg text-blue-500 text-[10px] font-black text-center" value={ep.season} onChange={e => { const u = [...editEpisodes]; u[idx].season = e.target.value; setEditEpisodes(u); }} />
                            <input className="flex-1 bg-slate-900 border border-slate-800 p-2 rounded-lg text-xs text-white font-bold" value={ep.title} placeholder="Title" onChange={e => { const u = [...editEpisodes]; u[idx].title = e.target.value; setEditEpisodes(u); }} />
                            <button onClick={() => setEditEpisodes(editEpisodes.filter((_, i) => i !== idx))}><Trash2 size={14} className="text-red-500"/></button>
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                            <input className="bg-slate-900 border border-slate-800 p-2 rounded-lg text-[9px] text-blue-400 font-mono" value={ep.tmdb_id} placeholder="TMDB Ep ID" onChange={e => { const u = [...editEpisodes]; u[idx].tmdb_id = e.target.value; setEditEpisodes(u); }} />
                            <input className="bg-slate-900 border border-slate-800 p-2 rounded-lg text-[9px] text-green-400 font-mono" value={ep.direct_url} placeholder="Stream URL" onChange={e => { const u = [...editEpisodes]; u[idx].direct_url = e.target.value; setEditEpisodes(u); }} />
                        </div>
                        <textarea className="w-full bg-slate-900 border border-slate-800 p-2 rounded-lg text-[9px] text-purple-400 font-mono" rows={1} value={ep.html} placeholder="Iframe Mirror Code..." onChange={e => { const u = [...editEpisodes]; u[idx].html = e.target.value; setEditEpisodes(u); }} />
                    </div>
                ))}
            </div>
        </div>
        ...
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Downloads ({editDownloads.length})</p>
                <button onClick={() => setEditDownloads([...editDownloads, { quality: "", size: "", links: [{ label: "", url: "" }] }])} className="text-blue-400 text-[9px] font-black uppercase">+ Quality</button>
            </div>
            <div className="space-y-6 max-h-[300px] overflow-y-auto pr-1 custom-scroll">
                {editDownloads.map((q, qIdx) => (
                <div key={qIdx} className="p-4 bg-slate-950 rounded-2xl border border-white/5 space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                        <input className="bg-slate-900 border border-slate-800 p-2 rounded-lg text-xs" placeholder="Quality (720p)" value={q.quality} onChange={(e) => { const u = [...editDownloads]; u[qIdx].quality = e.target.value; setEditDownloads(u); }} />
                        <input className="bg-slate-900 border border-slate-800 p-2 rounded-lg text-xs" placeholder="Size (1.2GB)" value={q.size} onChange={(e) => { const u = [...editDownloads]; u[qIdx].size = e.target.value; setEditDownloads(u); }} />
                    </div>
                    {q.links.map((l, lIdx) => (
                    <div key={lIdx} className="grid grid-cols-2 gap-3">
                        <input className="bg-slate-900 border border-slate-800 p-2 rounded-lg text-[10px]" placeholder="Label" value={l.label} onChange={(e) => { const u = [...editDownloads]; u[qIdx].links[lIdx].label = e.target.value; setEditDownloads(u); }} />
                        <input className="bg-slate-900 border border-slate-800 p-2 rounded-lg text-[10px] font-mono" placeholder="URL" value={l.url} onChange={(e) => { const u = [...editDownloads]; u[qIdx].links[lIdx].url = e.target.value; setEditDownloads(u); }} />
                    </div>
                    ))}
                    <div className="flex justify-between">
                        <button onClick={() => { const u = [...editDownloads]; u[qIdx].links.push({ label: "", url: "" }); setEditDownloads(u); }} className="text-green-400 text-[9px] font-black uppercase">+ Link</button>
                        <button onClick={() => setEditDownloads(editDownloads.filter((_, i) => i !== qIdx))} className="text-red-500 text-[9px] font-black uppercase">Remove</button>
                    </div>
                </div>
                ))}
            </div>
...
const UploadWatchHtml = () => {
  ...
  const [downloads, setDownloads] = useState([{ quality: "", size: "", links: [{ label: "", url: "" }] }]);
  ...
  const applyMetadata = (data) => {
    ...
    const ratingVal = data.imdb_rating ? Number(data.imdb_rating) : null;
        ...
        <div className="lg:col-span-7 space-y-8">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <section className="bg-slate-900/80 border border-blue-500/20 p-8 rounded-[2.5rem] flex items-center justify-between shadow-2xl">
                    ...
                    <button onClick={() => setFormData({...formData, show_on_hero: !formData.show_on_hero})} className={`w-16 h-8 rounded-full transition-all relative ${formData.show_on_hero ? 'bg-green-600' : 'bg-slate-800'}`}><div className={`absolute top-1.5 w-5 h-5 bg-white rounded-full shadow-lg transition-all ${formData.show_on_hero ? 'right-1.5' : 'left-1.5'}`} /></button>
                </section>
                <section className="bg-slate-900/80 border border-blue-500/20 p-8 rounded-[2.5rem] flex items-center justify-between shadow-2xl">
                    <div className="flex items-center gap-4">
                    ...
                    <button onClick={() => setFormData({...formData, is_trending: !formData.is_trending})} className={`w-16 h-8 rounded-full transition-all relative ${formData.is_trending ? 'bg-blue-600' : 'bg-slate-800'}`}><div className={`absolute top-1.5 w-5 h-5 bg-white rounded-full shadow-lg transition-all ${formData.is_trending ? 'right-1.5' : 'left-1.5'}`} /></button>
                </section>
            </div>

            <section className="bg-slate-900/80 border border-white/5 p-8 rounded-[2.5rem] space-y-6 shadow-xl relative overflow-hidden">
                <div className="flex items-center gap-2 border-b border-white/5 pb-4"><Layout size={18} className="text-blue-500"/><h3 className="font-black uppercase tracking-widest text-[11px] text-white">Identity</h3></div>
            ...
            <section className="bg-slate-900/80 border border-white/5 p-8 rounded-[2.5rem] space-y-6 shadow-xl text-white">
                <div className="flex items-center gap-2 border-b border-white/5 pb-4"><Film size={18} className="text-purple-500"/><h3 className="font-black uppercase tracking-widest text-[11px]">Visuals</h3></div>
                <div className="space-y-4">
            ...
            <section className="bg-slate-900/80 border border-white/5 p-8 rounded-[2.5rem] space-y-6 shadow-xl border-l-4 border-l-blue-600 text-white">
                <div className="flex items-center gap-2 border-b border-white/5 pb-4"><Monitor size={18} className="text-blue-400"/><h3 className="font-black uppercase tracking-widest text-[11px]">Global Stream</h3></div>
                <div className="space-y-4">
                    ...
                    <div className="flex gap-4 items-center bg-slate-800/30 p-4 rounded-2xl">
                        ...
                        <button onClick={() => handleVideoUpload(selectedFile)} disabled={isUploading || !selectedFile} className="bg-blue-600 px-6 py-2 rounded-xl font-black text-[10px] uppercase">{isUploading ? uploadProgress : "Upload"}</button>
                    </div>
                </div>
            ...
            <section className="bg-slate-900/80 border border-white/5 p-8 rounded-[2.5rem] space-y-6 shadow-xl text-white">
                <div className="flex items-center justify-between border-b border-white/5 pb-4">
                    <div className="flex items-center gap-2"><Tv size={18} className="text-blue-500"/><h3 className="font-black uppercase tracking-widest text-[11px]">Episodes</h3></div>
                    ...
                    {episodes.map((ep, idx) => (
                        <div key={idx} className="p-5 bg-slate-950 rounded-[2rem] border border-slate-800 space-y-4 group">
                            <div className="flex gap-3 items-center">
                                <input className="w-10 bg-slate-900 border border-slate-800 p-2 rounded-lg text-blue-500 text-[11px] font-black text-center" value={ep.season} onChange={e => { const u = [...episodes]; u[idx].season = e.target.value; setEpisodes(u); }} />
                                <input className="flex-1 bg-slate-900 border border-slate-800 p-2 rounded-lg text-xs text-white" placeholder="Episode Title" value={ep.title} onChange={e => { const u = [...episodes]; u[idx].title = e.target.value; setEpisodes(u); }} />
                                <button onClick={() => setEpisodes(episodes.filter((_, i) => i !== idx))} className="text-red-500"><Trash2 size={16}/></button>
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                                <input className="bg-slate-900 border border-slate-800 p-2.5 rounded-xl text-[10px] font-mono text-blue-400" placeholder="TMDB ID" value={ep.tmdb_id} onChange={e => { const u = [...episodes]; u[idx].tmdb_id = e.target.value; setEpisodes(u); }} />
                                <input className="bg-slate-900 border border-slate-800 p-2.5 rounded-xl text-[10px] text-green-400 font-mono" placeholder="Stream Link" value={ep.direct_url} onChange={e => { const u = [...episodes]; u[idx].direct_url = e.target.value; setEpisodes(u); }} />
                            </div>
                            <textarea className="w-full bg-slate-900 border border-slate-800 p-2.5 rounded-xl text-[10px] text-purple-400 font-mono" rows={1} placeholder="Mirror Code..." value={ep.html} onChange={e => { const u = [...episodes]; u[idx].html = e.target.value; setEpisodes(u); }} />
                        </div>
                    ))}
                </div>
            </section>
            ...
            <section className="bg-slate-900/80 border border-white/5 p-8 rounded-[2.5rem] space-y-6 shadow-xl text-white">
                <div className="flex items-center justify-between border-b border-white/5 pb-4">
                    <div className="flex items-center gap-2"><Database size={18} className="text-green-500" /><h3 className="font-black uppercase tracking-widest text-[11px]">Downloads</h3></div>
                    <button onClick={() => setDownloads([...downloads, { quality: "", size: "", links: [{ label: "", url: "" }] }])} className="text-[9px] font-black uppercase text-blue-400">+ Quality</button>
                </div>
                <div className="space-y-6">
                    {downloads.map((q, qIdx) => (
                    <div key={qIdx} className="bg-slate-950 border border-slate-800 p-5 rounded-2xl space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            <input placeholder="Quality (e.g. 720p)" className="bg-slate-900 border border-slate-800 p-2 rounded-xl text-xs" value={q.quality} onChange={(e) => { const u = [...downloads]; u[qIdx].quality = e.target.value; setDownloads(u); }} />
                            <input placeholder="Size (e.g. 1.2GB)" className="bg-slate-900 border border-slate-800 p-2 rounded-xl text-xs" value={q.size} onChange={(e) => { const u = [...downloads]; u[qIdx].size = e.target.value; setDownloads(u); }} />
                        </div>
                        {q.links.map((l, lIdx) => (
                        <div key={lIdx} className="grid grid-cols-2 gap-3">
                            <input placeholder="Label (Telegram / Direct)" className="bg-slate-900 border border-slate-800 p-2 rounded-xl text-[10px]" value={l.label} onChange={(e) => { const u = [...downloads]; u[qIdx].links[lIdx].label = e.target.value; setDownloads(u); }} />
                            <input placeholder="Download URL" className="bg-slate-900 border border-slate-800 p-2 rounded-xl text-[10px] font-mono" value={l.url} onChange={(e) => { const u = [...downloads]; u[qIdx].links[lIdx].url = e.target.value; setDownloads(u); }} />
                        </div>
                        ))}
                        <button onClick={() => { const u = [...downloads]; u[qIdx].links.push({ label: "", url: "" }); setDownloads(u); }} className="text-[9px] uppercase font-black text-green-400">+ Link</button>
                    </div>
                    ))}
                </div>
            </section>
        ...
        <div className="lg:col-span-5">
            <div className="sticky top-24 bg-slate-900/50 border border-white/5 p-8 rounded-[2.5rem] space-y-6 shadow-2xl backdrop-blur-2xl">
                ...
                <div className="mt-10 bg-slate-900/50 border border-white/5 p-6 rounded-[2.5rem] space-y-6 shadow-2xl">
                    <div className="flex items-center gap-2 border-b border-white/5 pb-4"><Database size={18} className="text-blue-500" /><h3 className="font-black uppercase tracking-widest text-[11px] text-white">Telegram Cloud Files</h3></div>
                    <div className="relative">
                        <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" />
                        <input value={tgSearch} onChange={(e) => setTgSearch(e.target.value)} placeholder="Search file name or code..." className="w-full bg-slate-950 border border-slate-800 pl-9 p-2 rounded-xl text-[10px] text-white outline-none focus:border-blue-500/40" />
                    </div>
                    {tgLoading && <div className="flex items-center gap-2 text-slate-400 text-xs"><Loader2 className="animate-spin" size={14} />Loading telegram files...</div>}
                    <div className="space-y-4 max-h-[45vh] overflow-y-auto custom-scroll pr-2">
                        {telegramFiles.filter((file) => `${file.file_name} ${file.file_code}`.toLowerCase().includes(tgSearch.toLowerCase())).map((file, index) => {
                            const url = `https://t.me/anchormovies_bot?start=${file.file_code}`;
                            return (
                                <div key={index} className="bg-slate-950 border border-slate-800 p-4 rounded-2xl hover:border-blue-500/40 transition-all space-y-2">
                                    <p className="text-xs font-bold text-white truncate">{file.file_name}</p>
                                    <div className="flex justify-between items-center text-[10px] font-mono"><span className="text-blue-400">Code: {file.file_code}</span><button onClick={() => navigator.clipboard.writeText(url)} className="text-green-400 hover:text-green-300 transition">Copy URL</button></div>
                                    <input readOnly value={url} className="w-full bg-slate-900 border border-slate-700 p-2 rounded-lg text-[9px] text-slate-300" />
                                </div>
                            );
                        })}
                    </div>
